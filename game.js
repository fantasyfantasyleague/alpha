...
const FACTIONS = {
  goblins: {
    ...
    heroes: {
      ...
      goblin_hexweaver: {
        ...
        startingEquipment: { mainhand: 'fire_wand', offhand: 'orb_of_storms' },
      ...
      goblin_ratking: {
        ...
        passives: ['pestilence'],
        startingEquipment: { mainhand: 'poisoned_dagger', offhand: 'buckler', accessory: 'scouts_compass' },
  ...
  humans: {
    ...
    heroes: {
      ...
      human_paladin: {
        ...
        ability: { id: 'consecrate', name: 'Consecrate', desc: 'Deal 2 damage and Weaken all enemies within 2 tiles. Lasts 3 turns.' },
  ...
  trolls: {
    ...
    heroes: {
      troll_warlord: {
        name: 'Warlord', char: '☰',
        hp: 27, movement: 4, attack: 4, defense: 3, str: 4, agi: 0, con: 0, int: 0,
        passives: ['hunger'],
...
const ITEMS = {
  ...
  serpent_crown:         { id: 'serpent_crown',         name: 'Serpent Crown',        slot: 'head',     stats: { defense: 1, int: 1 },              weight: 1, shopCost: 7, onHit: { effect: 'poison', chance: 100 }, description: '+1 INT. Venomous crown. 100% chance to inflict Poison on hit.' },
  ...
  poisoned_dagger:      { id: 'poisoned_dagger',      name: 'Poisoned Dagger',       slot: 'mainhand', stats: { attack: 2, agi: 2 },              weight: 1, shopCost: 8, onHit: { effect: 'poison', chance: 100 }, description: '+2 AGI. Venomous edge. 50% chance to inflict Poison.' },
  ...
  venom_fang:            { id: 'venom_fang',            name: 'Venom Fang',            slot: 'mainhand', stats: { attack: 2, agi: 3 },              weight: 1, shopCost: 9, onHit: { effect: 'poison', chance: 100, stacks: 2 }, passives: ['piercing'], description: '+3 AGI. Razor fang dripping venom. 60% Poison (2 stacks). Grants Piercing.' },
  ...
  // Poison weapons
  dart_gun:              { id: 'dart_gun',              name: 'Dart Gun',              slot: 'mainhand', stats: { attack: 2, agi: 2 },              weight: 1, shopCost: 9, range: 3, onHit: { effect: 'poison', chance: 100 }, description: '+2 AGI. Blowpipe loaded with toxic darts. Ranged (3). 100% Poison. One-handed.' },
  plague_glaive:         { id: 'plague_glaive',         name: 'Plague Glaive',         slot: 'mainhand', stats: { attack: 5, str: 2, agi: 1 },     weight: 4, shopCost: 13, twoHanded: true, onHit: { effect: 'poison', chance: 100, stacks: 3 }, passives: ['potent_formula'], description: '+2 STR, +1 AGI. Festering polearm. 100% Poison (2 stacks). Grants Potent Formula. Two-handed.' },
  venomcleaver:          { id: 'venomcleaver',          name: 'Venomcleaver',          slot: 'mainhand', stats: { attack: 3, str: 2 },              weight: 3, shopCost: 10, combatLockBonus: 15, onHit: { effect: 'poison', chance: 100, stacks: 2 }, description: '+2 STR. Toxin-laced waraxe. 100% Poison (2 stacks). +15% combat lock chance.' },
  // Ranged Magic Weapons
  gnarled_staff:        { id: 'gnarled_staff',        name: 'Gnarled Staff',         slot: 'mainhand', stats: {},                                weight: 1, shopCost: 1, twoHanded: true, range: 2, magicDamage: true, description: 'A gnarled staff crackling with unstable arcane threads. Ranged (2). Two-handed.' },
  ...
  orb_of_storms:         { id: 'orb_of_storms',         name: 'Orb of Storms',         slot: 'offhand',  stats: { int: 2, attack: 1 },              weight: 1, shopCost: 8, onHit: { effect: 'slowed', chance: 100 }, description: '+2 INT. Crackles with lightning. 100% chance to Slow.' },
  ...
  orb_of_poison:         { id: 'orb_of_poison',         name: 'Orb of Poison',         slot: 'offhand',  stats: {},                                  weight: 1, shopCost: 8, onHit: { effect: 'poison', chance: 100, stacks: 2 }, description: 'Drips with venom. Attacks inflict Poison (2 stacks).' },
  ...
  grippers:              { id: 'grippers',              name: 'Grippers',               slot: 'hands',    stats: {},                                   weight: 0, shopCost: 11, passives: ['dual_grip'], description: 'No stats. Allows equipping an offhand item while wielding a two-handed weapon. Grants Strong Grip.' },
...
const STATUS_EFFECTS = {
  ...
  dark_energy:   { id: 'dark_energy',   name: 'Dark Energy',   icon: '🌑', color: '#6600CC', duration: 3, stackable: true, maxStacks: 5, desc: '+1 STR and +1 AGI per stack (max 5). Lasts 3 turns. Critting refreshes all stacks.' },
};
...
const PASSIVES = {
  ...
  dark_purpose:          { id: 'dark_purpose',          name: 'Dark Purpose',          icon: '🌑', desc: 'Removes crit cap (up to 100%). On crit with a magic weapon or spell, gain 2 stacks of Dark Energy (+1 STR/AGI per stack, max 5, 3 turns). Critting refreshes all stacks.' },
...
function getUnitStats(unit) {
  ...
  let movement = (severedLegs || hasStatusEffect(unit, 'frozen') || hasStatusEffect(unit, 'ensnared') || hasStatusEffect(unit, 'combat_locked')) ? 0 : Math.max(2, unit.movement + agiBonus - weightPenalty + seMods.move + (hasPassive(unit, 'haste') ? 2 : 0) + (hasPassive(unit, 'mounted') ? 5 : 0) + unarmoredMoveBonus + itemMoveBonus);
  ...
  const dodgeChance = hasPassive(unit, 'lumbering_oaf') ? 0 : (hasStatusEffect(unit, 'greased') ? 100 : Math.max(0, Math.min(50, totalAgi * 2 + (hasPassive(unit, 'evasion') ? 10 : 0) + bardicDodgeBonus - discordantDodgePenalty))); // AGI × 2%, cap 50%, greased = 100%, lumbering_oaf = always 0
  const critCap = hasPassive(unit, 'dark_purpose') ? 100 : 40;
  const critChance = Math.max(0, Math.min(critCap, totalInt * 5 + itemCritBonus - jinxPenalty)); // INT × 5% + item crit bonus - jinx, cap 40% (100% with Dark Purpose)
  ...
  for (let y = 0; y < G.boardSize; y++) {
    for (let x = 0; x < G.boardSize; x++) {
      ...
      for (const cu of consecrateUnits) {
        ...
        if (dist <= 2) {
          if (chebyshevDist(x, y - 1, cu.x, cu.y) > 2) cell.style.borderTop = '3px dotted #DAA520';
          if (chebyshevDist(x, y + 1, cu.x, cu.y) > 2) cell.style.borderBottom = '3px dotted #DAA520';
          if (chebyshevDist(x - 1, y, cu.x, cu.y) > 2) cell.style.borderLeft = '3px dotted #DAA520';
          if (chebyshevDist(x + 1, y, cu.x, cu.y) > 2) cell.style.borderRight = '3px dotted #DAA520';
        }
      }
...
function performAttack(attacker, defender, { isAoO = false, isDoubleStrike = false } = {}) {
  ...
  if (USE_NEW_COMBAT) {
    ...
    const hitTarget = Math.max(2, Math.min(6, 4 - Math.floor((atkPower - effectiveDef) / 2)));
    ...
    damage = woundRoll >= woundTarget ? Math.max(1, 1 + (woundRoll - woundTarget)) : 0;
...
function resolveAttack(attacker, defender) {
  ...
  // NPC chase toward attacker when hit at range (melee retaliation handled by retaliate passive)
...
function executeConsecrate(unit) {
  // Consecrate: deal 2 damage and Weaken all enemies within 2 tiles. Lasts 3 turns.
  const enemies = G.units.filter(u =>
  ...
  for (const enemy of enemies) {
    ...
    addLog(`✝ Consecrate hits ${getDisplayName(enemy)} for 2 damage! Weakened! (${enemy.hp > 0 ? enemy.hp + ' HP left' : 'DEFEATED!'})`);
    if (enemy.hp <= 0) {
      enemy.hp = 0;
      dropAllItems(enemy);
...
function triggerDarkPurpose(unit) {
  ...
  if (existing) {
    ...
    existing.turnsLeft = 3; // reset decay timer on every crit
    if (existing.stacks > oldStacks) {
      addLog(`🌑 ${getDisplayName(unit)}'s Dark Purpose surges! Dark Energy ${existing.stacks}/5 stacks (+${existing.stacks} STR/AGI, 3 turns)`);
    } else {
      addLog(`🌑 ${getDisplayName(unit)}'s Dark Purpose refreshes! Dark Energy at max stacks (5/5, 3 turns)`);
    }
  } else {
    unit.statusEffects.push({ id: 'dark_energy', turnsLeft: 3, stacks: 2 });
    addLog(`🌑 ${getDisplayName(unit)}'s Dark Purpose awakens! Dark Energy 2/5 stacks (+2 STR/AGI, 3 turns)`);
  }
}
...
function autoGatherAll() {
  ...
  for (const worker of workers) {
    // 1. If worker hasn't gathered yet and is adjacent to a resource, gather the nearest one
    if (!worker.hasGathered) {
      const adjRes = getAdjacentResources(worker);
      if (adjRes.length > 0) {
        // Pick closest (they're all adjacent so pick first available)
        const target = adjRes[0];
        gatherResource(worker, target.x, target.y);
        gathered++;
        continue;
      }
    }

    // 2. If worker still has movement left, move towards the nearest resource on the board
    if (worker.movementLeft > 0) {
      const nearest = findNearestResource(worker);
      if (!nearest) continue; // no resources left on the board

      // Find the best reachable tile that gets closest to the resource
      const reachable = getReachableTiles(worker);
      if (reachable.length === 0) continue;

      // Sort by chebyshev distance to the target resource, pick closest
      reachable.sort((a, b) => {
        const da = chebyshevDist(a.x, a.y, nearest.x, nearest.y);
        const db = chebyshevDist(b.x, b.y, nearest.x, nearest.y);
        return da - db;
      });

      const bestTile = reachable[0];
      const currentDist = chebyshevDist(worker.x, worker.y, nearest.x, nearest.y);
      const bestDist = chebyshevDist(bestTile.x, bestTile.y, nearest.x, nearest.y);

      // Only move if it actually gets us closer
      if (bestDist < currentDist) {
        moveUnit(worker, bestTile.x, bestTile.y);
        moved++;

        // After moving, try to gather if now adjacent
        if (!worker.hasGathered && worker.hp > 0) {
          const adjRes = getAdjacentResources(worker);
          if (adjRes.length > 0) {
            gatherResource(worker, adjRes[0].x, adjRes[0].y);
            gathered++;
          }
        }
      }
...
function equipItem(unitId, inventoryIndex) {
  ...
  // With Dual Wield + Strong Grip: can also dual-wield two-handed weapons
  if (slot === 'mainhand' && hasPassive(u, 'dual_wield') &&
      u.equipment.mainhand && u.equipment.mainhand !== '_two_handed_') {
    // 1H weapons always work with dual wield
...
function confirmEndTurn() {
  ...
  for (const u of G.units) {
    if (u.playerId === G.currentPlayer && u.hp > 0) {
      ...
      // Consecrate aura tick: 2 damage + weaken enemies within 2 tiles each turn
      if (u.consecrateTurns && u.consecrateTurns > 0) {
        ...
        for (const enemy of consecEnemies) {
          enemy.hp -= 2;
          applyStatusEffect(enemy, 'weakened', 2);
          addLog(`✝ Consecrated ground hits ${getDisplayName(enemy)} for 2 damage! Weakened! (${enemy.hp > 0 ? enemy.hp + ' HP left' : 'DEFEATED!'})`);
          if (enemy.hp <= 0) {
            enemy.hp = 0;
            dropAllItems(enemy);
            addLog(`☠ ${getDisplayName(enemy)} has been slain!`);
...
function saveGame() {
  ...
  a.href = url;
  a.download = `${G.players[G.currentPlayer].name.replace(/\s+/g, '_')}_turn-${G.turn}_ALPHA.json`;
  a.click();
